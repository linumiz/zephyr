...
static int strip_optional_field(char *json, const char *key,
...
static int extract_string_field(char *out_buf, int outlen, const char *token)
{
	if (!out_buf || !token) {
		return -EINVAL;
	}
			      ...
			      const char *pdumsg)
{
	...
	const char *action = ocpp_get_pdu_literal(pdu);
	if (!action) {
		return -EINVAL;
	}
				...
				struct ocpp_session *ses)
{
	...
	const struct json_obj_descr authorize_descr[] = {
			       ...
			       struct ocpp_cp_info *cpi)
{
	...
	int ret = 0;
	const struct json_obj_descr bootnotif_descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_bootnotif_msg,
					 "chargePointModel",
					 charge_point_model,
					 JSON_TOK_STRING),
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_bootnotif_msg,
	...
	struct ocpp_bootnotif_msg msg = {
		...
		.charge_box_serial_number = cpi->box_sl_no ?
					    cpi->box_sl_no : "NULL",
		.charge_point_serial_number = cpi->sl_no ?
					      cpi->sl_no : "NULL",
		.firmware_version = cpi->fw_ver ? cpi->fw_ver : "NULL",
		.iccid = cpi->iccid ? cpi->iccid : "NULL",
		.imsi = cpi->imsi ? cpi->imsi : "NULL",
		.meter_serial_number = cpi->meter_sl_no ?
				       cpi->meter_sl_no : "NULL",
	...
	if (strcmp(msg.firmware_version, "NULL") == 0) {
		strip_optional_field(tmp_buf, "firmwareVersion",
				     STRIP_STRING);
	};
	...
	if (strcmp(msg.charge_box_serial_number, "NULL") == 0) {
		strip_optional_field(tmp_buf, "chargeBoxSerialNumber",
				     STRIP_STRING);
	};
	if (strcmp(msg.charge_point_serial_number, "NULL") == 0) {
		strip_optional_field(tmp_buf, "chargePointSerialNumber",
				     STRIP_STRING);
	};
	if (strcmp(msg.meter_serial_number, "NULL") == 0) {
		strip_optional_field(tmp_buf, "meterSerialNumber",
				     STRIP_STRING);
	};
...
static int frame_meter_val_msg(char *buf, int len, struct ocpp_session *ses,
			       const char *timestamp, const char *val,
			       const char *measurand, const char *unit)
{
	int ret = 0;
	char tmp_buf[MAX_METERVALUE_REQ_LEN];

	const struct json_obj_descr sampled_value_descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_sampled_value,
					  "measurand", measurand,
					  JSON_TOK_STRING),
	...
	const struct json_obj_descr meter_value_descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_meter_value, "timestamp",
					  timestamp, JSON_TOK_STRING),
		JSON_OBJ_DESCR_OBJ_ARRAY_NAMED(struct ocpp_meter_value,
	...
	const struct json_obj_descr meter_val_msg_descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_meter_val_msg,
					  "connectorId",
					  connector_id, JSON_TOK_NUMBER),
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_meter_val_msg,
					  "transactionId",
					  transaction_id, JSON_TOK_NUMBER),
		JSON_OBJ_DESCR_OBJ_ARRAY_NAMED(struct ocpp_meter_val_msg,
				              "meterValue", meter_value, 1,
					      meter_value_len,
					      meter_value_descr,
...
static int frame_stop_txn_msg(char *buf, int len, struct ocpp_session *ses,
			      int Wh, const char *reason,
			      const char *timestamp)
{
	int ret = 0;

	const struct json_obj_descr descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_stop_txn_msg,
...
static int frame_start_txn_msg(char *buf, int len, struct ocpp_session *ses,
			       int Wh, int reserv_id, const char *timestamp)
{
	int ret = 0;
	char tmp_buf[MAX_STARTTRANSACTION_REQ_LEN];

	const struct json_obj_descr descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_start_txn_msg,
...
static int frame_getconfig_msg(char *buf, int len,
			       ...
			       bool is_rw, char *uid)
{
	...
	const struct json_obj_descr keyval_descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_key_val,
					  "key", key, JSON_TOK_STRING),
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_key_val,
					  "readonly", readonly,
					  JSON_TOK_NUMBER),
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_key_val,
					  "value", value, JSON_TOK_STRING),
	};

	const struct json_obj_descr config_descr[] = {
		JSON_OBJ_DESCR_OBJ_ARRAY_NAMED(struct ocpp_getconfig_msg,
					       "configurationKey",
		                               configuration_key, 1,
					       configuration_key_len,
		                               keyval_descr,
					       ARRAY_SIZE(keyval_descr)),
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_getconfig_msg,
...
static int frame_status_resp_msg(char *buf, int len, const char *res,
				 char *uid)
{
	int ret = 0;
	...
	const struct json_obj_descr descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct ocpp_status_resp_msg,
					  "status", status, JSON_TOK_STRING),
	};
...
static int parse_idtag_info(char *json, struct ocpp_idtag_info *idtag_info)
{
	...
	const struct json_obj_descr inner_descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct idtag_info_inner, "status",
					  status, JSON_TOK_STRING),
		JSON_OBJ_DESCR_PRIM_NAMED(struct idtag_info_inner,
	...
	const char *status = parsed.id_tag_info.status;
	if (!status) {
		return -EINVAL;
	}
...
static int parse_heartbeat_msg(char *json, struct timeval *date)
{
	int ret = 0;
	const struct json_obj_descr descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct heartbeat_payload,
...
static int parse_bootnotification_msg(char *json, struct boot_notif *binfo)
{
	...
	const struct json_obj_descr descr[] = {
		...
		JSON_OBJ_DESCR_PRIM_NAMED(struct bootnotif_payload,
					  "currentTime", current_time,
					  JSON_TOK_STRING),
	};
	...
	ret = json_obj_parse(json, strlen(json), descr, ARRAY_SIZE(descr),
			     &msg);
	if (ret < 0) {
		return ret;
	}
			       ...
			       struct ocpp_idtag_info *idtag_info)
{
	...
	const struct json_obj_descr descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct start_txn_payload,
					  "transactionId", transaction_id,
					  JSON_TOK_NUMBER),
	};
				      ...
				      char *idtag)
{
	...
	const struct json_obj_descr descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct remote_start_txn_payload,
					 "idTag", id_tag, JSON_TOK_STRING),
		JSON_OBJ_DESCR_PRIM_NAMED(struct remote_start_txn_payload,
					 "connectorId", connector_id,
					 JSON_TOK_NUMBER),
	};
...
static int parse_remote_stop_txn_msg(char *json, int *idtxn)
{
	...
	const struct json_obj_descr descr[] = {
		JSON_OBJ_DESCR_PRIM_NAMED(struct remote_stop_txn_payload,
					  "transactionId", transaction_id,
					  JSON_TOK_NUMBER),
	};
...
static int parse_unlock_connectormsg(char *json, int *idcon)
{
	...
	const struct json_obj_descr descr[] = {
	JSON_OBJ_DESCR_PRIM_NAMED(struct unlock_connector_payload,
				  "connectorId", connector_id,
				  JSON_TOK_NUMBER),
};
