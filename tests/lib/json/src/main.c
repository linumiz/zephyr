...
ZTEST(lib_json_test, test_json_mixed_arr_parse)
{
	...
	char encoded[] = "[\"msg\", 123456, {\"nested_int\":42,\"nested_bool\":true,\"nested_string\":\"abc\",\"nested_string_buf\":\"buf\",\"nested_int8\":1,\"nested_uint8\":2,\"nested_int64\":3,\"nested_uint64\":4}, [10,20,30], \"ok\"]";
	int ret = json_mixed_arr_parse(encoded, strlen(encoded),
		test_mixed_arr_descr, ARRAY_SIZE(test_mixed_arr_descr), &arr);
...
ZTEST(lib_json_test, test_json_mixed_arr_encode)
{
	...
	int ret = json_mixed_arr_encode_buf(test_mixed_arr_descr,
		ARRAY_SIZE(test_mixed_arr_descr), &arr, buf, sizeof(buf));

	zassert_equal(ret, 0, NULL);

	ret = json_mixed_arr_parse(buf, strlen(buf),
		test_mixed_arr_descr, ARRAY_SIZE(test_mixed_arr_descr), &pkt);
