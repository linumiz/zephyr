...
static void cat1a_srom_responseip_isr(void *arg)
{
	uint32_t masked = 0;
...
static int cat1a_srom_init()
{
	/* Initialize SROM response interrupt*/
	IRQ_CONNECT(CY_SROM_DR_IPC_INTR_NO, 2, cat1a_srom_responseip_isr, NULL, 0);
...
SYS_INIT(cat1a_srom_init, PRE_KERNEL_1, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT);
