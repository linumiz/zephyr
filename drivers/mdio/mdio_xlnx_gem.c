...
static int xlnx_gem_mdio_initialize(const struct device *dev)
{
	...
	uint32_t freq = Cy_SysClk_ClkMemGetFrequency();

	printk("Mem clock %u\n", freq);
	freq = Cy_SysClk_ClkSlowGetFrequency();
	printk("slow clock %u\n", freq);
	...
	uint32_t *ptr = 0x40480000;
	reg_val = 0x80000002;  /* enable rgmii mode and enable eth ip block */

	sys_write32(reg_val, ptr);
