...
#define LOG_MODULE_NAME eth_cdns_gem
#define LOG_LEVEL CONFIG_ETHERNET_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(LOG_MODULE_NAME);

static void eth_cdns_gem_iface_init(struct net_if *iface);
static int  eth_cdns_gem_send(const struct device *dev, struct net_pkt *pkt);
static int  eth_cdns_gem_start_device(const struct device *dev, struct net_if *iface);
static int  eth_cdns_gem_stop_device(const struct device *dev, struct net_if *iface);
static enum ethernet_hw_caps
	eth_cdns_gem_get_capabilities(const struct device *dev, struct net_if *iface);
static int  eth_cdns_gem_get_config(const struct device *dev,
				    struct net_if *iface,
				    enum ethernet_config_type type,
				    struct ethernet_config *config);
static int eth_cdns_gem_set_config(const struct device *dev,
				   struct net_if *iface,
				   enum ethernet_config_type type,
				   const struct ethernet_config *config);
static const struct device *eth_cdns_gem_get_phy(const struct device *dev,
						 struct net_if *iface);
#ifdef CONFIG_NET_STATISTICS_ETHERNET
static struct net_stats_eth *eth_cdns_gem_get_stats(const struct device *dev,
						struct net_if *iface);
#endif

static void eth_cdns_gem_reset_hw(const struct device *dev);
static void eth_cdns_gem_set_initial_nwcfg(const struct device *dev);
static void eth_cdns_gem_set_mac_address(const struct device *dev);
static void eth_cdns_gem_set_initial_dmacr(const struct device *dev);
static void eth_cdns_gem_configure_buffers(const struct device *dev);
static void eth_cdns_gem_rx_pending_work(struct k_work *item);
static void eth_cdns_gem_handle_rx_pending(const struct device *dev);
static void eth_cdns_gem_tx_done_work(struct k_work *item);
static void eth_cdns_gem_handle_tx_done(const struct device *dev);
static void eth_cdns_gem_set_nwcfg_link_speed(const struct device *dev,
					      struct phy_link_state *state);
static void eth_cdns_gem_phy_cb(const struct device *phy,
				struct phy_link_state *state,
				void *eth_dev);

const struct ethernet_api eth_cdns_gem_apis = {
	.iface_api.init   = eth_cdns_gem_iface_init,
	.get_capabilities = eth_cdns_gem_get_capabilities,
	.get_phy	  = eth_cdns_gem_get_phy,
	.send		  = eth_cdns_gem_send,
	.start		  = eth_cdns_gem_start_device,
	.stop		  = eth_cdns_gem_stop_device,
	.get_config	  = eth_cdns_gem_get_config,
	.set_config	  = eth_cdns_gem_set_config,
#ifdef CONFIG_NET_STATISTICS_ETHERNET
	.get_stats	  = eth_cdns_gem_get_stats,
#endif
};
...
{
	const struct eth_cdns_gem_dev_cfg *dev_conf __maybe_unused = DEV_CFG(dev);
	int ret;
	...
	ret = eth_cdns_gem_platform_init(dev);
	if (ret < 0) {
		return ret;
	}
	 ...
	 * (if applicable).
	 */
	eth_cdns_gem_reset_hw(dev);		/* Chapter 16.3.1 */
	eth_cdns_gem_set_initial_nwcfg(dev);	/* Chapter 16.3.2 */
	eth_cdns_gem_set_mac_address(dev);	/* Chapter 16.3.2 */
	eth_cdns_gem_set_initial_dmacr(dev);	/* Chapter 16.3.2 */
	eth_cdns_gem_configure_buffers(dev);	/* Chapter 16.3.5 */
...
static void eth_cdns_gem_iface_init(struct net_if *iface)
{
	...
	const struct eth_cdns_gem_dev_cfg *dev_conf = DEV_CFG(dev);
	struct eth_cdns_gem_dev_data *dev_data = DEV_DATA(dev);
	int ret;
	 ...
	 * handling.
	 */
	k_work_init(&dev_data->tx_done_work, eth_cdns_gem_tx_done_work);
	k_work_init(&dev_data->rx_pend_work, eth_cdns_gem_rx_pending_work);
	...
	ret = phy_link_callback_set(dev_conf->phy_dev, eth_cdns_gem_phy_cb, (void *)dev);
	if (ret) {
...
void eth_cdns_gem_isr(const struct device *dev)
{
	...
	reg_val = sys_read32(DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_ISR_OFFSET);

	/*
	 * TODO: handling if one or more error flag(s) are set in the
	 * interrupt status register. -> For now, just log them
	 */
	if (reg_val & ETH_CDNS_GEM_IXR_ERRORS_MASK) {
	 ...
	 */
	if ((reg_val & ETH_CDNS_GEM_IXR_TX_COMPLETE_BIT) != 0) {
		sys_write32(ETH_CDNS_GEM_IXR_TX_COMPLETE_BIT,
			    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_IDR_OFFSET);
		sys_write32(ETH_CDNS_GEM_IXR_TX_COMPLETE_BIT,
			    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_ISR_OFFSET);
		if (dev_conf->defer_txd_to_queue) {
			k_work_submit(&dev_data->tx_done_work);
		} else {
			eth_cdns_gem_handle_tx_done(dev);
		}
	}
	if ((reg_val & ETH_CDNS_GEM_IXR_FRAME_RX_BIT) != 0) {
		sys_write32(ETH_CDNS_GEM_IXR_FRAME_RX_BIT,
			    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_IDR_OFFSET);
		sys_write32(ETH_CDNS_GEM_IXR_FRAME_RX_BIT,
			    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_ISR_OFFSET);
		if (dev_conf->defer_rxp_to_queue) {
			...
			eth_cdns_gem_handle_rx_pending(dev);
		}
	}

	/*
	 * Clear all interrupt status bits so that the interrupt is de-asserted
	 * by the GEM. -> TXSR/RXSR are read/cleared by either eth_cdns_gem_-
	 * handle_tx_done or eth_cdns_gem_handle_rx_pending if those actions
	 ...
	 */
	sys_write32((0xFFFFFFFF & ~(ETH_CDNS_GEM_IXR_FRAME_RX_BIT |
		    ETH_CDNS_GEM_IXR_TX_COMPLETE_BIT)),
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_ISR_OFFSET);
}
...
static int eth_cdns_gem_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	if (bds_reqd > dev_data->tx_bd_ring.free_bds) {
		...
		if (dev_conf->defer_txd_to_queue) {
			...
			sys_write32(ETH_CDNS_GEM_IXR_TX_COMPLETE_BIT,
				    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_IER_OFFSET);
		}
#ifdef CONFIG_NET_STATISTICS_ETHERNET
	...
	do {
		...
		reg_val = sys_read32(reg_ctrl) & (ETH_CDNS_GEM_TX_BD_WRAP_BIT |
			  ETH_CDNS_GEM_TX_BD_USED_BIT);
	...
	reg_val |= ETH_CDNS_GEM_TX_BD_LAST_BIT;
	 ...
	 */
	reg_val &= ~ETH_CDNS_GEM_TX_BD_USED_BIT;
	sys_write32(reg_val, reg_ctrl);
#ifdef CONFIG_DCACHE
	...
	reg_val  = sys_read32(DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_NWCTRL_OFFSET);
	reg_val |= ETH_CDNS_GEM_NWCTRL_STARTTX_BIT;
	sys_write32(reg_val, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_NWCTRL_OFFSET);
 ...
 */
static int eth_cdns_gem_start_device(const struct device *dev,
				     struct net_if *iface __unused)
{
	...
	sys_write32(ETH_CDNS_GEM_IXR_ALL_MASK,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_IDR_OFFSET);
	sys_write32(ETH_CDNS_GEM_IXR_ALL_MASK,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_ISR_OFFSET);

	/* Clear RX & TX status registers */
	sys_write32(0xFFFFFFFFU, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_TXSR_OFFSET);
	sys_write32(0xFFFFFFFFU, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_RXSR_OFFSET);

	/* RX and TX enable */
	reg_val  = sys_read32(DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_NWCTRL_OFFSET);
	reg_val |= (ETH_CDNS_GEM_NWCTRL_RXEN_BIT | ETH_CDNS_GEM_NWCTRL_TXEN_BIT);
	sys_write32(reg_val, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_NWCTRL_OFFSET);
 ...
 * @retval    0 upon successful completion
 */
static int eth_cdns_gem_stop_device(const struct device *dev,
				    struct net_if *iface __unused)
{
	struct eth_cdns_gem_dev_data *dev_data = DEV_DATA(dev);
	uint32_t reg_val;
 ...
 */
static enum ethernet_hw_caps eth_cdns_gem_get_capabilities(
				   ...
				   const struct ethernet_config *config)
{
	...
	case ETHERNET_CONFIG_TYPE_PROMISC_MODE:
		uint32_t reg_val = sys_read32(DEVICE_MMIO_NAMED_GET(dev, mac) +
					      ETH_CDNS_GEM_NWCFG_OFFSET);

		if (config->promisc_mode) {
			reg_val |= ETH_CDNS_GEM_NWCFG_COPYALLEN_BIT;
		} else {
			reg_val &= ~ETH_CDNS_GEM_NWCFG_COPYALLEN_BIT;
		}
		sys_write32(reg_val, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_NWCFG_OFFSET);
		break;
#endif
	case ETHERNET_CONFIG_TYPE_MAC_ADDRESS:
		memcpy(dev_data->mac_addr, config->mac_address.addr, sizeof(dev_data->mac_addr));
		eth_cdns_gem_set_mac_address(dev);
		break;
	default:
		return -ENOTSUP;
	};
...
static void eth_cdns_gem_reset_hw(const struct device *dev)
{
	 ...
	 */
	nwctrl = sys_read32(DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_NWCTRL_OFFSET);
	nwctrl &= ETH_CDNS_GEM_NWCTRL_MDEN_BIT;
	nwctrl |= ETH_CDNS_GEM_NWCTRL_STATCLR_BIT; /* clear statistics counters */
	sys_write32(nwctrl, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_NWCTRL_OFFSET);

	/* Clear the RX/TX status registers */
	sys_write32(ETH_CDNS_GEM_TXSRCLR_MASK,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_TXSR_OFFSET);
	sys_write32(ETH_CDNS_GEM_RXSRCLR_MASK,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_RXSR_OFFSET);

	/* Disable all interrupts */
	sys_write32(ETH_CDNS_GEM_IDRCLR_MASK,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_IDR_OFFSET);

	/* Clear the buffer queues */
	sys_write32(0x00000000,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_RXQBASE_OFFSET);
	sys_write32(0x00000000,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_TXQBASE_OFFSET);
#ifdef CONFIG_SOC_XILINX_ZYNQMP
	sys_write32(0x00000000,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_RX1QBASEL_OFFSET);
	sys_write32(0x00000000,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_RX1QBASEH_OFFSET);
	sys_write32(0x00000000,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_TX1QBASEL_OFFSET);
	sys_write32(0x00000000,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_TX1QBASEH_OFFSET);
#endif
}
 ...
 * register. This is called from within #eth_cdns_gem_poll_phy.
 *
 * @param dev Pointer to the device data
					      ...
					      struct phy_link_state *state)
{
	 ...
	 */
	reg_val  = sys_read32(DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_NWCFG_OFFSET);
	reg_val &= ~(ETH_CDNS_GEM_NWCFG_1000_BIT |
		     ETH_CDNS_GEM_NWCFG_100_BIT |
		     ETH_CDNS_GEM_NWCFG_FDEN_BIT);

	/* No bits to set for 10 Mbps. 100 Mbps and 1 Gbps set one bit each. */
	if (PHY_LINK_IS_SPEED_100M(state->speed)) {
		reg_val |= ETH_CDNS_GEM_NWCFG_100_BIT;
...
static void eth_cdns_gem_set_mac_address(const struct device *dev)
{
	...
	sys_write32(regval_bot, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_LADDR1L_OFFSET);
	sys_write32(regval_top, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_LADDR1H_OFFSET);
...
static void eth_cdns_gem_set_initial_dmacr(const struct device *dev)
{
	...
	if (dev_conf->disc_rx_ahb_unavail) {
		...
		reg_val |= ETH_CDNS_GEM_DMACR_DISCNOAHB_BIT;
	}
	/*
	...
	reg_val |= (((dev_conf->rx_buffer_size / 64) &
		   ETH_CDNS_GEM_DMACR_RX_BUF_MASK) <<
		   ETH_CDNS_GEM_DMACR_RX_BUF_SHIFT);
	if (!dev_conf->disable_tx_chksum_offload) {
		/* [11] TX TCP/UDP/IP checksum offload to GEM */
		reg_val |= ETH_CDNS_GEM_DMACR_TCP_CHKSUM_BIT;
	}
	if (dev_conf->tx_buffer_size_full) {
		/* [10] TX buffer memory size select */
		reg_val |= ETH_CDNS_GEM_DMACR_TX_SIZE_BIT;
	}
	/*
	...
	reg_val |= ((uint32_t)dev_conf->ahb_burst_length &
		   ETH_CDNS_GEM_DMACR_AHB_BURST_LENGTH_MASK);

	/* Write the assembled register contents */
	sys_write32(reg_val, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_DMACR_OFFSET);
}
...
static void eth_cdns_gem_configure_buffers(const struct device *dev)
{
	const struct eth_cdns_gem_dev_cfg *dev_conf = DEV_CFG(dev);
	struct eth_cdns_gem_dev_data *dev_data = DEV_DATA(dev);
	struct eth_cdns_gem_bd *bdptr;
	uint32_t buf_iter;

	/* Initial configuration of the RX/TX BD rings */
	dev_data->rx_bd_ring.first_bd = dev_conf->rx_bd_ring_base;
	dev_data->rx_bd_ring.tie_off_bd = dev_conf->rx_tie_off_bd;
	dev_data->tx_bd_ring.first_bd = dev_conf->tx_bd_ring_base;
	dev_data->tx_bd_ring.tie_off_bd = dev_conf->tx_tie_off_bd;
	dev_data->first_rx_buffer = dev_conf->rx_buf_base;
	dev_data->first_tx_buffer = dev_conf->tx_buf_base;
	dev_data->rx_tie_off_buffer = dev_conf->rx_tie_buf_base;
	dev_data->tx_tie_off_buffer = dev_conf->tx_tie_buf_base;
	...
	for (buf_iter = 0; buf_iter < (dev_conf->rx_bd_count - 1); buf_iter++) {
		...
		bdptr->addr = addr & ~(ETH_CDNS_GEM_RX_BD_USED_BIT | ETH_CDNS_GEM_RX_BD_WRAP_BIT);
	...
	uint32_t last_rx_addr = (uint32_t)dev_data->first_rx_buffer +
				(buf_iter * dev_conf->rx_buffer_size);
	bdptr->addr = (((uint32_t)last_rx_addr) & ~ETH_CDNS_GEM_RX_BD_USED_BIT) |
		      ETH_CDNS_GEM_RX_BD_WRAP_BIT;
	bdptr->ctrl = 0x00000000;
	...
	for (buf_iter = 0; buf_iter < (dev_conf->tx_bd_count - 1); buf_iter++) {
		...
		bdptr->ctrl = ETH_CDNS_GEM_TX_BD_USED_BIT;
		++bdptr;
	}

	bdptr->addr = (uint32_t)dev_data->first_tx_buffer +
		      (buf_iter * (uint32_t)dev_conf->tx_buffer_size);
	bdptr->ctrl = (ETH_CDNS_GEM_TX_BD_WRAP_BIT | ETH_CDNS_GEM_TX_BD_USED_BIT);

	/*
	 * Configure the tie-off dummy buffer descriptors.
	...
	bdptr->addr = (uint32_t)dev_data->rx_tie_off_buffer | ETH_CDNS_GEM_RX_BD_USED_BIT |
		      ETH_CDNS_GEM_RX_BD_WRAP_BIT;
	bdptr->ctrl = 0x00000000;
	...
	sys_write32((uint32_t)dev_data->tx_bd_ring.first_bd,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_TXQBASE_OFFSET);
#ifdef CONFIG_SOC_XILINX_ZYNQMP
	sys_write32(0x00000000, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_RX1QBASEH_OFFSET);
	sys_write32((uint32_t)dev_data->rx_bd_ring.tie_off_bd,
		    DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_RX1QBASEL_OFFSET);
	sys_write32(0x00000000, DEVICE_MMIO_NAMED_GET(dev, mac) + ETH_CDNS_GEM_TX1QBASEH_OFFSET);
 ...
 * Wraps the RX data pending handler, eth_cdns_gem_handle_rx_pending,
...
static void eth_cdns_gem_rx_pending_work(struct k_work *item)
{
	struct eth_cdns_gem_dev_data *dev_data = CONTAINER_OF(item,
		struct eth_cdns_gem_dev_data, rx_pend_work);
...
static void eth_cdns_gem_handle_rx_pending(const struct device *dev)
{
	...
	while (1) {
		...
		if ((reg_val & ETH_CDNS_GEM_RX_BD_USED_BIT) == 0) {
			/*
		...
		reg_val = sys_read32(reg_ctrl);
		if ((reg_val & ETH_CDNS_GEM_RX_BD_START_OF_FRAME_BIT) == 0) {
			/*
		...
		do {
			...
			rx_data_length = rx_data_remaining =
					 (reg_val & ETH_CDNS_GEM_RX_BD_FRAME_LENGTH_MASK);
			if ((reg_val & ETH_CDNS_GEM_RX_BD_END_OF_FRAME_BIT) == 0) {
				last_bd_idx = (last_bd_idx + 1) % dev_conf->rx_bd_count;
			}
		} while ((reg_val & ETH_CDNS_GEM_RX_BD_END_OF_FRAME_BIT) == 0);
		...
		do {
			if (pkt != NULL) {
				...
				sys_cache_data_invd_range(
					...
					ETH_CDNS_GEM_RX_BD_BUFFER_ADDR_MASK),
					dev_conf->rx_buffer_size);
#endif
			...
			reg_val	 = sys_read32(reg_addr);
			reg_val &= ~ETH_CDNS_GEM_RX_BD_USED_BIT;
 ...
 * Wraps the TX done handler, eth_cdns_gem_handle_tx_done,
 * for the scenario in which the current GEM device is configured
 * to defer RX pending / TX done indication handling to the system
 * work queue. In this case, the work item received by this wrapper
 * function will be enqueued from within the ISR if the corresponding
...
{
	struct eth_cdns_gem_dev_data *dev_data = CONTAINER_OF(item,
		struct eth_cdns_gem_dev_data, tx_done_work);
	const struct device *dev = net_if_get_device(dev_data->iface);
 ...
 * execution, the eth_cdns_gem_send call which effectively triggered
...
static void eth_cdns_gem_handle_tx_done(const struct device *dev)
{
	...
	/* Indicate completion to a blocking eth_cdns_gem_send() call */
				...
				void *eth_dev)
{
	...
	if (state->is_up) {
		eth_cdns_gem_set_link_speed(dev, state);
		eth_cdns_gem_set_nwcfg_link_speed(dev, state);
	}
