...
cy_rslt_t ifx_cat1_uart_set_baud(const struct device *dev, uint32_t baudrate)
{
	...
	status = clock_control_set_rate(config->clk_dev, &data->clk_info,
					&peri_frequency);
...
#if ((CONFIG_SOC_FAMILY_INFINEON_CAT1C) && !defined(CONFIG_INFINEON_TRAVEO_FAMILY))
#define IRQ_INFO(n)                                                                                \
...
#define IRQ_INFO(n) .irq_num = DT_INST_IRQN(n), .irq_priority = DT_INST_IRQ(n, priority)
    ...
    !defined(CONFIG_INFINEON_TRAVEO_FAMILY)) || defined(CONFIG_SOC_FAMILY_INFINEON_EDGE)
