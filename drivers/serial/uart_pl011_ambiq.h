...
static inline int pl011_ambiq_clk_set(const struct device *dev, uint32_t clk)
{
	...
	case 24000000:
		...
		break;
#if !defined(CONFIG_SOC_SERIES_APOLLO3X) && !defined(CONFIG_SOC_SERIES_APOLLO2X)
	case 48000000:
...
#define AMBIQ_PL011_UART_DEFINE(n)                                                                 \
	static void pwr_on_ambiq_pl011_uart_##n(void)                                              \
	{                                                                                          \
		uint32_t module = (DT_INST_REG_ADDR(n) - UART0_BASE) / (UART1_BASE - UART0_BASE);  \
		uint32_t eUARTPowerModule =                                                        \
			((uint32_t)(AM_HAL_PWRCTRL_UART0 << module));                              \
		       am_hal_pwrctrl_periph_enable(eUARTPowerModule);                             \
	}                                                                                          \
