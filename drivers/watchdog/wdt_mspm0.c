						...
						const struct wdt_timeout_cfg *cfg)
{
	...
	uint32_t period_count_interval;
	uint32_t proposed_interval;
	 ...
	 */
	proposed_interval = period_count_interval;
	for(int i = 0; i < 8; i++){
		if(max_ms <= proposed_interval){
			data->clockDivider = i;
			break;
		}
	...
	for(int i = 0; i < 8; i++){
		if(min_ms <= (proposed_interval * window_sixteenth[i]/16)) {
...
static int wwdt_mspm0_setup(const struct device *dev, uint8_t options)
{
	...
	DL_WWDT_initWatchdogMode((WWDT_Regs *) config->base, data->clockDivider,
			data->periodCount, sleep_mode, data->windowCount0, data->windowCount1);
