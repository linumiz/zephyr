...
static int offload_connect(void *obj, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	if(!net_if_is_carrier_ok(mdata.netif)){
		LOG_ERR("Netwrok conenction not available");
		return -ENOTCONN;
	}
	...
	ret = snprintk(buf, sizeof(buf), SIMCOM_CMD_CONNECT, sock->sock_fd,
								#if defined CONFIG_MODEM_SIM7080
				...
				struct sockaddr *src_addr, socklen_t *addrlen)
{
	...
	snprintk(sendbuf, sizeof(sendbuf), SIMCOM_CMD_DATA_RECV, sock->sock_fd, max_len);
...
static void socket_close(struct modem_socket *sock)
{
	...
	snprintk(buf, sizeof(buf), SIMCOM_CMD_SOCKET_CLOSE, sock->sock_fd);
