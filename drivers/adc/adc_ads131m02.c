...
/* ADS131M02 cmds */
#define ADS131M02_NULL			0x0000
#define ADS131M02_RESET			0x0011
#define ADS131M02_STANDBY		0x0022
#define ADS131M02_WAKEUP		0x0033
#define ADS131M02_LOCK			0x0555
#define ADS131M02_UNLOCK		0x0655
#define ADS131M02_RREG			0xA000
#define ADS131M02_WREG			0x6000

/* ADS131M02 Power modes */
#define ADS131M02_PM_HR			BIT(1)
#define ADS131M02_PM_LP			BIT(0)
#define ADS131M02_PM_VLP		0
...
struct ads131m02_config {
	...
	const struct gpio_dt_spec gpio_sync_reset;
};
...
static inline int ads131m02_check_device(const struct spi_dt_spec *dev)
{
	return spi_is_ready_dt(dev);
}
			      ...
			      uint8_t *buf, size_t size)
{
	...
	temp = (uint16_t)(ADS131M02_RREG | (addr << 7) | 0);
	sys_put_be16(temp, tx_buffer);
			       ...
			       uint16_t data)
{
	...
	temp = (uint16_t)(ADS131M02_WREG | (addr << 7) | 0);
	sys_put_be16(temp, tx_buffer);
	sys_put_be16(data, &tx_buffer[3]);
					   ...
					   const struct adc_channel_cfg *channel_cfg)
{
	...
	case ADC_GAIN_1:
		gain_cfg = ADS131M02_GET_GAIN(channel_cfg->channel_id, ADS131M02_GAIN_1);
		break;
	case ADC_GAIN_2:
		gain_cfg = ADS131M02_GET_GAIN(channel_cfg->channel_id, ADS131M02_GAIN_2);
		break;
	case ADC_GAIN_4:
		gain_cfg = ADS131M02_GET_GAIN(channel_cfg->channel_id, ADS131M02_GAIN_4);
		break;
	case ADC_GAIN_8:
		gain_cfg = ADS131M02_GET_GAIN(channel_cfg->channel_id, ADS131M02_GAIN_8);
		break;
	case ADC_GAIN_16:
		gain_cfg = ADS131M02_GET_GAIN(channel_cfg->channel_id, ADS131M02_GAIN_16);
		break;
	case ADC_GAIN_32:
		gain_cfg = ADS131M02_GET_GAIN(channel_cfg->channel_id, ADS131M02_GAIN_32);
		break;
	case ADC_GAIN_64:
		gain_cfg = ADS131M02_GET_GAIN(channel_cfg->channel_id, ADS131M02_GAIN_64);
		break;
	case ADC_GAIN_128:
		gain_cfg = ADS131M02_GET_GAIN(channel_cfg->channel_id, ADS131M02_GAIN_128);
		break;
	default:
		return -EINVAL;
	}
...
static inline int ads131m02_acquistion_time(uint16_t acq_time)
{
	uint16_t acq_value = ADC_ACQ_TIME_VALUE(acq_time);

	if (acq_time == ADC_ACQ_TIME_DEFAULT) {
		return ADS131M02_OSR_1024_MASK;
	}
	...
	if (acq_time == ADC_ACQ_TIME_MAX) {
		return ADS131M02_OSR_16384_MASK;
	}
	...
	case ADS131M02_DR_500:
		return ADS131M02_OSR_8192_MASK;
	case ADS131M02_DR_1k:
		return ADS131M02_OSR_4096_MASK;
	case ADS131M02_DR_2k:
		return ADS131M02_OSR_2048_MASK;
	case ADS131M02_DR_4k:
		return ADS131M02_OSR_1024_MASK;
	case ADS131M02_DR_8k:
		return ADS131M02_OSR_512_MASK;
	case ADS131M02_DR_16k:
		return ADS131M02_OSR_256_MASK;
	case ADS131M02_DR_32k:
		return 0;
	default:
		return -EINVAL;
	}
			   ...
			   const struct adc_channel_cfg *channel_cfg)
{
	...
	ret = ads131m02_acquistion_time(channel_cfg->acquisition_time);
	if (ret < 0) {
		return ret;
	}
				 ...
				 uint32_t channels, int32_t *buffer)
{
	...
	for (int i = 0; i < 12; i++) {
		printk("0x%x ", buffer_rx[i]);
	}

	int_status = (uint16_t)sys_get_be16(&buffer_rx[0]);

	if ((int_status & ADS131M02_DRDY_CH0_MASK) && (channels & BIT(0))) {
		*buffer = sys_get_be24(&buffer_rx[3]);
	} else if ((int_status & ADS131M02_DRDY_CH1_MASK) && (channels & BIT(1))) {
		*buffer = sys_get_be24(&buffer_rx[6]);
	} else
...
static int ads131m02_read(const struct device *dev, const struct adc_sequence *seq)
{
	int ret;
	struct ads131m02_data *data =  dev->data;
...
static int ads131m02_configure_gpio(const struct device *dev)
{
	...
	if (cfg->gpio_sync_reset.port != NULL) {
		ret = gpio_pin_configure_dt(&cfg->gpio_sync_reset, GPIO_OUTPUT_INACTIVE);
		if (ret != 0) {
			return ret;
		}
	}
...
static int ads131m02_device_reset(const struct device *dev)
{
	...
	if (cfg->gpio_sync_reset.port == NULL) {
		LOG_WRN("sync reset gpio is not configured");
		return 0;
	}

	ret = gpio_pin_set_dt(&cfg->gpio_sync_reset, 1);
	if (ret != 0) {
		return ret;
	}
	...
	ret = gpio_pin_set_dt(&cfg->gpio_sync_reset, 0);
	if (ret != 0) {
		return ret;
	}
...
static int ads131m02_init(const struct device *dev)
{
	...
	if (!ads131m02_check_device(&cfg->spi)) {
	...
	if (ret != 0) {
		LOG_ERR("Failed to read ADS131M02 %d", ret);
		return ret;
	}
	...
	ret = ads131m02_reg_write(dev, ADS131M02_CLOCK_REG, ADS131M02_DISABLE_ADC);
	if (ret != 0) {
...
#define ADC_ADS131M02_INST_DEFINE(n)							\
	static const struct ads131m02_config config_##n = {				\
		...
		.gpio_sync_reset = GPIO_DT_SPEC_INST_GET(n, sync_reset_gpios),		\
	};										\
	static struct ads131m02_data data_##n;						\
	DEVICE_DT_INST_DEFINE(n, ads131m02_init, NULL,					\
