...
static int32_t ads131m02_normalize_count24(uint8_t *tReg)
{
	int32_t s32AdcCount = sys_get_be24(tReg);

	/* adc data is given in binary two's complement format */
	/* for negative values - fill leading bits with 1 (sign extension) */
	if ((((uint32_t)s32AdcCount) & 0x00800000UL) == 0x00800000UL)
	{
		s32AdcCount = (int32_t)(((uint32_t)s32AdcCount) | 0xFF800000UL);
	}
	return s32AdcCount;
}
