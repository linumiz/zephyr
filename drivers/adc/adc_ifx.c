...
static int ifx_cat1_sar_read_internal(const struct device *dev, const struct adc_sequence *sequence)
{
	struct ifx_cat1_sar_data *data = dev->data;
	size_t exp_size;
	...
	if (sequence->buffer_size < exp_size) {
		LOG_ERR("Buffer too small: need %zu bytes, have %zu", exp_size,
...
static int ifx_cat1_sar_read(const struct device *dev, const struct adc_sequence *sequence)
{
	struct ifx_cat1_sar_data *data = dev->data;
	int ret;
...
static int ifx_cat1_sar_read_async(const struct device *dev, const struct adc_sequence *sequence,
...
#else
	clock_control_get_rate(DEVICE_DT_GET(DT_NODELABEL(clk_hf2)), NULL, &hf_clock_frequency);
#endif
