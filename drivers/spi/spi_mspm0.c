...
#define MSPM0_MAIN_CLK_DIV(n)  CONCAT(DL_SPI_CLOCK_DIVIDE_RATIO_, DT_INST_PROP(n, ti_clk_div))
#define MSPM0_SPI_INIT(inst)                                                                       \
	...
	static struct spi_mspm0_config spi_mspm0_##inst##_cfg = {                                  \
		...
		.clock_config = {.clockSel = MSPM0_CLOCK_PERIPH_REG_MASK(DT_INST_CLOCKS_CELL(inst, clk)),                                  \
				 .divideRatio = MSPM0_MAIN_CLK_DIV(inst)},                      \
	...
	static struct spi_mspm0_data spi_mspm0_##inst##_data = {                                   \
		...
		SPI_CONTEXT_CS_GPIOS_INITIALIZE(DT_DRV_INST(inst), ctx)};                          \
                                                                                                   \
	SPI_DEVICE_DT_INST_DEFINE(inst, spi_mspm0_init, NULL, &spi_mspm0_##inst##_data,                \
