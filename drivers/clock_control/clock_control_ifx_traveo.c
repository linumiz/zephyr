...
#define CONFIG_OSC(clock)							\
	do {									\
		ret = ifx_configure_oscillators(dev, DT_REG_ADDR(clock));	\
		if (ret != 0) {							\
			return ret;						\
		}								\
	} while(0)
...
static inline int ifx_clk_get_rate(const struct device *dev,
...
#else
		Cy_SysClk_EcoSetFrequency(DT_PROP(DT_NODELABEL(eco),
					  clock_frequency));
		return 0;
#endif
	default:
...
static int ifx_configure_pll(const struct ifx_pll_cfg *pll_configs)
{
	...
	for (int i = 0; i < IFX_NUM_PLL; i++) {
		...
		ret = Cy_SysClk_ClkPathSetSource(pll_cfg->clkpath,
						 pll_cfg->clksource);
		if (ret != 0) {
			return ret;
		}
		...
		ret = Cy_SysClk_PllManualConfigure(pll_cfg->clkpath,
						   &pll_cfg->pll_config);
		if (ret != 0) {
			return ret;
		}

		ret = Cy_SysClk_PllEnable(pll_cfg->clkpath,
...
static int ifx_configure_hf_clk(const struct ifx_hf_clk_cfg *hf_configs)
{
	...
	for (int i = 0; i < IFX_NUM_HF_CLK; i++) {
		...
		ret = Cy_SysClk_ClkHfSetSource(hf_cfg->hf_clk_id,
					       hf_cfg->source);
		if (ret != 0) {
			return ret;
		}

		ret = Cy_SysClk_ClkHfSetDivider(hf_cfg->hf_clk_id,
						hf_cfg->divider);
		if (ret != 0) {
			return ret;
		}
...
static int ifx_clk_init(const struct device *dev)
{
	...
	DT_FOREACH_CHILD_STATUS_OKAY(DT_PATH(clocks, oscillators), CONFIG_OSC);

#if defined(CONFIG_CPU_CORTEX_M0PLUS)
	const struct ifx_clk_cfg *cfg = (struct ifx_clk_cfg *)dev->config;
	...
	Cy_SysClk_ClkSlowSetDivider(DT_PROP(DT_NODELABEL(clk_hf0),
					    slow_clk_divider));
#endif
...
#define IFX_CLOCK_PLL_CONFIG(node_id)                                           \
        {                                                                       \
                ...
                .pll_config     = {                                             \
                        ...
                        .fracEn         = DT_PROP_OR(node_id, frac_div, 0),     \
                },                                                              \
        }
...
DEVICE_DT_DEFINE(DT_NODELABEL(clocks), &ifx_clk_init, NULL,
		 NULL, &ifx_clk_config, PRE_KERNEL_1,
		 CONFIG_CLOCK_CONTROL_INIT_PRIORITY,
		 &ifx_clock_control_api);
