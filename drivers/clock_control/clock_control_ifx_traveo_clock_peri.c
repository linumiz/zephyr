						 ...
						 uint32_t *rate)
{
	...
	*rate = Cy_SysClk_PeriPclkGetFrequency(cfg->peri_clk_inst,
					       clk->divider_type,
					       clk->divider_inst);
						 ...
						 clock_control_subsys_rate_t rate)
{
	...
	ret = Cy_SysClk_PeriPclkDisableDivider(clock_id,
					       clk->divider_type,
					       clk->divider_inst);
	if (ret != 0) {
		goto out;
	...
	case IFX_PERI_DIV_8:
	case IFX_PERI_DIV_16:
		ret = Cy_SysClk_PeriPclkSetDivider(clock_id,
						   clk->divider_type,
						   clk->divider_inst,
						   int_div);
		break;
	case IFX_PERI_DIV_16_5:
	case IFX_PERI_DIV_24_5:
		uint8_t frac_div = (((float)data->src_clk_freq / (float) *clk_rate) \
		...
		ret = Cy_SysClk_PeriPclkSetFracDivider(clock_id,
						       clk->divider_type,
						       clk->divider_inst,
						       int_div, frac_div);
		break;
	...
	if (ret != 0) {
        ...
        ret = Cy_SysClk_PeriPclkEnableDivider(clock_id, clk->divider_type,
					      clk->divider_inst);
        if (ret != 0) {
		goto out;
        }
...
static int clock_control_ifx_clock_peri_init(const struct device *dev)
{
	...
	ret = clock_control_get_rate(DEVICE_DT_GET(DT_NODELABEL(clocks)),
...
#define FILL_DIVIDER_INST(inst) 						\
	.max_dividers[IFX_PERI_DIV_8] = DT_INST_PROP_OR(inst,			\
						ifx_8_bit_max_divider, 0),	\
	.max_dividers[IFX_PERI_DIV_16] = DT_INST_PROP_OR(inst,			\
						ifx_16_bit_max_divider, 0),	\
	.max_dividers[IFX_PERI_DIV_16_5] = DT_INST_PROP_OR(inst,		\
						ifx_16_5_bit_max_divider, 0),	\
	.max_dividers[IFX_PERI_DIV_24_5] = DT_INST_PROP_OR(inst,		\
						ifx_8_bit_max_divider, 0),
...
#define IFX_CLOCK_PERI_INIT(inst)						\
	static const struct ifx_clock_peri_config clk_cfg_##inst = {		\
		...
		FILL_DIVIDER_INST(inst)						\
										...
										\
	struct ifx_clock_peri_data clk_data_##inst;				\
										\
