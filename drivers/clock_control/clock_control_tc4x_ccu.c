...
#include <zephyr/dt-bindings/clock/tc4xx_clock.h>
...
#define DT_DRV_COMPAT infineon_tc4xx_ccu

struct clock_control_tc4xx_divider {
	uint32_t fasclinsi_div: 4;
	uint32_t fasclinf_div: 4;
};
struct clock_control_tc4xx_data {
	uint32_t fsource0;
	uint32_t fsource1;
	uint32_t fsource2;
	uint32_t fsource3;

	struct clock_control_tc4xx_divider divider;
};

static inline int clock_control_tc4xx_ccu_osc_init()
{
...
static inline int clock_control_tc4xx_ccu_syspll_default()
{
...
static inline int clock_control_tc4xx_ccu_syspll_power(bool enable)
{
...
static inline int clock_control_tc4xx_ccu_syspll_init()
{
...
static inline int clock_control_tc4xx_ccu_syspll_divider()
{
	Ifx_CLOCK_SYSPLLCON1 con1 = CLOCK_SYSPLLCON1;
...
static inline int clock_control_tc4xx_ccu_perpll_default()
{
...
static inline int clock_control_tc4xx_ccu_perpll_power(bool enable)
{
...
static inline int clock_control_tc4xx_ccu_perpll_init()
{
...
static inline int clock_control_tc4xx_ccu_perpll_divider()
{
	Ifx_CLOCK_PERPLLCON1 con1 = CLOCK_PERPLLCON1;
...
static inline void clock_control_tc4xx_ccu_set_sys_source(enum sys_clock_source src)
{
...
static inline void clock_control_tc4xx_ccu_set_per_source(enum per_clock_source src)
{
	CLOCK_CCUCON.B.CLKSELP = src;
}

static inline int clock_control_tc4xx_ccu_ramposc_init()
{
...
static inline int clock_control_tc4xx_ccu_ramposc_move(bool to_top)
{
...
static inline int clock_control_tc4xx_ccu_set_divider()
{
	/* clang-format off */
...
static int clock_control_tc4xx_ccu_on(const struct device *dev, clock_control_subsys_t sys)
{
...
static int clock_control_tc4xx_ccu_off(const struct device *dev, clock_control_subsys_t sys)
{

	return -ENOSYS;
}

static enum clock_control_status clock_control_tc4xx_ccu_get_status(const struct device *dev,
								    clock_control_subsys_t sys)
{
	return -ENOSYS;
}

static int clock_control_tc4xx_ccu_get_rate(const struct device *dev, clock_control_subsys_t sys,
					    uint32_t *rate)
{
	uint32_t clock_id = *((uint32_t *)sys);
	struct clock_control_tc4xx_data *data = dev->data;
...
static int clock_control_tc4xx_ccu_init(const struct device *dev)
{
	int ret;
	...
	if (CLOCK_IS_CLOCK_SOURCE(fosc, FOSC_SOURCES)) {
		ret = clock_control_tc4xx_ccu_osc_init();
		if (ret) {
			return ret;
		}
	}
	...
	if (CLOCK_SOURCE_IS(fsource0, fback)) {
		clock_control_tc4xx_ccu_set_sys_source(SYS_SOURCE_FBACK);
	} else {
		ret = clock_control_tc4xx_ccu_ramposc_init();
		if (ret) {
			return ret;
		}
		clock_control_tc4xx_ccu_set_sys_source(SYS_SOURCE_FRAMP);
	}

	if (CLOCK_SOURCE_IS(fsource0, fpll0)) {
		/* TODO: Check for App Reset and Clock config */
		ret = clock_control_tc4xx_ccu_syspll_power(false);
		if (ret) {
			return ret;
		}

		ret = clock_control_tc4xx_ccu_syspll_default();
		if (ret) {
			return ret;
		}

		ret = clock_control_tc4xx_ccu_syspll_init();
		if (ret) {
			return ret;
		}
	}
	...
	clock_control_tc4xx_ccu_set_per_source(PER_SOURCE_FBACK);

	if (CLOCK_SOURCE_IS(fsource1, fpll1)) {
		/* TODO: Check for App Reset and Clock config */
		ret = clock_control_tc4xx_ccu_perpll_power(false);
		if (ret) {
			return ret;
		}

		ret = clock_control_tc4xx_ccu_perpll_default();
		if (ret) {
			return ret;
		}

		ret = clock_control_tc4xx_ccu_perpll_init();
		if (ret) {
			return ret;
		}
	}

	ret = clock_control_tc4xx_ccu_set_divider();
	if (ret) {
		return ret;
	}

	ret = clock_control_tc4xx_ccu_ramposc_move(true);
	if (ret) {
		return ret;
	}
	...
	ret = clock_control_tc4xx_ccu_syspll_divider();
	if (ret) {
		return ret;
	}
	clock_control_tc4xx_ccu_set_sys_source(SYS_SOURCE_PLL);

	ret = clock_control_tc4xx_ccu_perpll_divider();
	if (ret) {
		return ret;
	}
	clock_control_tc4xx_ccu_set_per_source(PER_SOURCE_PLL);

	return 0;
}

static int clock_control_tc4xx_ccu_init_check(const struct device *dev)
{

	return 0;
}

#if CONFIG_CLOCK_CONTROL_TC4XX_CCU_INIT_CHECK_ONLY
#define CCU_INIT clock_control_tc4xx_ccu_init_check
#else
#define CCU_INIT clock_control_tc4xx_ccu_init
#endif

static struct clock_control_tc4xx_data clock_control_tc4xx_data = {
...
static const struct clock_control_driver_api clock_control_tc4xx_ccu_api = {
	.on = clock_control_tc4xx_ccu_on,
	.off = clock_control_tc4xx_ccu_off,
	.get_rate = clock_control_tc4xx_ccu_get_rate,
	.get_status = clock_control_tc4xx_ccu_get_status,
};

DEVICE_DT_INST_DEFINE(0, &CCU_INIT, NULL, &clock_control_tc4xx_data, NULL, PRE_KERNEL_1,
		      CONFIG_CLOCK_CONTROL_INIT_PRIORITY, &clock_control_tc4xx_ccu_api);
