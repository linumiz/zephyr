...
static int fps_read_sys_param(const struct device *dev, struct r502a_sys_param *val)
{
	...
	val->status_reg = sys_get_be16(&rx_packet.data[1]);
	val->system_id = sys_get_be16(&rx_packet.data[3]);
	val->lib_size = sys_get_be16(&rx_packet.data[5]);
	val->sec_level = sys_get_be16(&rx_packet.data[7]);
	val->addr = sys_get_be32(&rx_packet.data[9]);
	offset = (sys_get_be16(&rx_packet.data[13]) + 5);
	val->data_pkt_size = r502a_pow(2, offset);
	val->baud = sys_get_be16(&rx_packet.data[15]) * 9600;
...
static int fps_store_model(const struct device *dev, uint8_t char_buf_idx,
								uint16_t id)
{
	struct grow_r502a_data *drv_data = dev->data;
	union r502a_packet rx_packet = {0};
...
static int fps_match_templates(const struct device *dev)
{
	...
	if (rx_packet.buf[R502A_CC_IDX] == R502A_OK) {
		fps_led_control(dev, &led_ctrl);
		drv_data->matching_score = sys_get_be16(&rx_packet.data[1]);
			       ...
			       enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_R502A_CAPTURE:
		return fps_get_image(dev);
	case SENSOR_ATTR_R502A_CONVERT:
		return fps_image_to_char(dev, val->val1);
	case SENSOR_ATTR_R502A_RECORD_CREATE:
		return fps_create_model(dev);
	case SENSOR_ATTR_R502A_RECORD_ADD:
		return fps_store_model(dev, val->val1, val->val2);
