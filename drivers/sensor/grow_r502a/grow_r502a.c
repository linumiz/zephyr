...
static int fps_led_control(const struct device *dev, struct led_params *led_control)
{
	...
	union r502a_packet tx_packet = {
		...
		.data = { R502A_LED_CONFIG, led_control->color_code,
				led_control->speed, led_control->color, led_control->cycle}
	};
...
static int fps_get_image(const struct device *dev)
{
	...
	if (rx_packet.buf[R502A_CC_IDX] == R502A_OK) {
		...
		led_ctrl.color_code = LED_CTRL_ON_ALWAYS;
...
static int fps_image_to_char(const struct device *dev, uint8_t slot)
{
	struct grow_r502a_data *drv_data = dev->data;
	union r502a_packet rx_packet = {0};
...
static int fps_store_model(const struct device *dev, uint16_t id)
{
	...
	if (rx_packet.buf[R502A_CC_IDX] == R502A_OK) {
		led_ctrl.color = LED_COLOR_BLUE;
		led_ctrl.color_code = LED_CTRL_FLASHING;
		led_ctrl.cycle = 0x03;
		fps_led_control(dev, &led_ctrl);
...
static int fps_search(const struct device *dev, uint8_t slot)
{
	struct grow_r502a_data *drv_data = dev->data;
	union r502a_packet rx_packet = {0};
	...
	if (rx_packet.buf[R502A_CC_IDX] == R502A_OK) {
		led_ctrl.color_code = LED_CTRL_FLASHING;
		led_ctrl.color = LED_COLOR_PURPLE;
		led_ctrl.cycle = 0x01;
		fps_led_control(dev, &led_ctrl);
		...
		led_ctrl.color_code = LED_CTRL_BREATHING;
		led_ctrl.color = LED_COLOR_RED;
		led_ctrl.cycle = 0x02;
		fps_led_control(dev, &led_ctrl);
			       ...
			       enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_RECORD_ADD:
		return fps_enroll(dev, val);
	case SENSOR_ATTR_RECORD_DEL:
		return fps_delete(dev, val);
	case SENSOR_ATTR_RECORD_EMPTY:
			       ...
			       enum sensor_attribute attr, struct sensor_value *val)
{
	...
	switch ((enum sensor_attribute_grow_r502a)attr) {
	case SENSOR_ATTR_RECORD_FIND:
		ret = fps_match(dev, val);
		break;
	case SENSOR_ATTR_RECORD_FREE_IDX:
...
#ifdef CONFIG_GROW_R502A_GPIO_POWER
